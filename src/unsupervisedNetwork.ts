...
export class UnsupervisedNN {
    ...
    input_size: number = 51
    first_hidden_layer_size = 64;
    hidden_layer_size: number = 32;
    latent_layer_size: number = 8;
    frameCounter: number = 0;
    trainingInterval: number = 20;
    experienceBuffer: number[][] = [];
    bufferSize: number = 100;
    
    W1: number[][]; //64 x 51 used for encoder
    W2: number[][]; //32 x 64 used for encoder
    W3: number[][]; //8 x 32 used for encoder
    W4: number[][]; //32 x 8 used for decoder
    W5: number[][]; //64 x 32 used for decoder
    W6: number[][]; //51 x 64 used for decoder
    ...
    private getDirectionalFeatures(): number[] {
        ...
        for (let i = 0; i < this.cell.neighbourhood.length; i++) {
            ...
            if (!tile || tile.length < 3) continue;
        ...
        for (const dir of directions) {
            ...
            features.push(
                ...
                isFinite(normalizedHeat) ? normalizedHeat : 0,
                isFinite(normalizedPheromone) ? normalizedPheromone : 0,
                isFinite(weightedFood) ? weightedFood : 0
            );
        }
    ...
    private getRadialFeatures(): number[] {
        ...
        for (const ring of rings) {
            ...
            features.push(
                isFinite(maxFood) ? maxFood : 0,
                isFinite(normalizedHeat) ? normalizedHeat : 0,
                isFinite(pheromoneDensity) ? pheromoneDensity : 0
            );
        }
    ...
    private inspectWeights(): void {
        const maxWeight = Math.max(
            ...this.W1.flat().map(Math.abs),
            ...this.W2.flat().map(Math.abs),
            ...this.W3.flat().map(Math.abs)
        );
        
        if (maxWeight > 10) {
            console.warn(`Cell ${this.cell?.id}: Weights exploding! Max: ${maxWeight}`);
